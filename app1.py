...
# -------------------- SUBTITLE EXTRACTION --------------------
def extract_subtitles_yt_dlp(youtube_url,retries=3):
    for f in os.listdir():
        if f.endswith(".vtt"):
            os.remove(f)
...
def index():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            "ml": "ml-IN",  # Malayalam
            "mr": "mr-IN",  # Marathi
            "od": "or-IN",  # Odia (ISO code is 'or', not 'od')
            "pa": "pa-IN",  # Punjabi
            "ta": "ta-IN",  # Tamil
            }
            ...
            print("End-To-End Pipeline time: ", pipeline_end_time-pipeline_start_time)
